...
import random
import secrets
from speck64_cipher import encrypt_speck64_128
...
def make_sample_real_single(delta, key, rounds, plaintext=None):
    ...
    C = encrypt_speck64_128(P, key, rounds)
    Cp = encrypt_speck64_128(P ^ delta, key, rounds)

    return ct_to_bits(C ^ Cp).reshape(1, 64)


def make_sample_random_single(key, rounds):
    C = encrypt_speck64_128(random.getrandbits(64), key, rounds)
    Cp = encrypt_speck64_128(random.getrandbits(64), key, rounds)
...
def make_sample_real_multi(
    ...
    for d in deltas:
        Pi = P0 ^ d
        Ci = encrypt_speck64_128(Pi, key, rounds)
        ct_list.append(Ci)
...
if __name__ == "__main__":
    ...
    key = int(args.key, 0) if args.key is not None else secrets.randbits(128)

    print("SPECK-64/128 dataset generator")
    ...
    if args.mode in ["single", "both"]:
        ...
        fname = f"speck64_R1_r{rounds}_s{run_seed}.npz"
    ...
    if args.mode in ["multi", "both"]:
        ...
        fname = f"speck64_R2_r{rounds}_s{run_seed}.npz"
