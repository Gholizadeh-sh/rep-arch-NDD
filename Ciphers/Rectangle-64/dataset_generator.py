...
def make_sample_real_single(delta, key, rounds, plaintext=None):
    ...
    C = encrypt_rect64_80(P, key, rounds)
    Cp = encrypt_rect64_80(P ^ delta, key, rounds)

    return ct_to_bits(C ^ Cp).reshape(1, 64)


def make_sample_random_single(key, rounds):
    C = encrypt_rect64_80(random.getrandbits(64), key, rounds)
    Cp = encrypt_rect64_80(random.getrandbits(64), key, rounds)
...
def generate_dataset_single(
    ...
    for i in range(n_real):
        ...
        X[idx] = make_sample_real_single(delta, key, rounds, pt)
        y[idx] = 1
        idx += 1

    for _ in range(n_random):
        X[idx] = make_sample_random_single(key, rounds)
        idx += 1
...
def make_sample_real_multi(
    ...
    for d in deltas:
        Pi = P0 ^ d
        Ci = encrypt_rect64_80(Pi, key, rounds)
        ct_list.append(Ci)
...
def generate_dataset_multi(
    ...
    probe = make_sample_real_multi(deltas, key, rounds)
...
if __name__ == "__main__":
    ...
    print("RECTANGLE-64 dataset generator")
    ...
    if args.mode in ["single", "both"]:
        ...
        fname = f"rect64_R1_r{rounds}_s{run_seed}.npz"
    ...
    if args.mode in ["multi", "both"]:
        ...
        fname = f"rect64_R2_r{rounds}_s{run_seed}.npz"
